function [mean_z,mean_z2,upp_bound] = Algorithm1(T,s,Com_G,a_star,H,test_no,sigma,N,b)
...
while m<=test_no
    ...
    x(:,i,k+1) = y(:,i,k) - sum(Com_G.W(i,:).*diff,2)- s* H(:,i)*(H(:,i)'*y(:,i,k)-(b(i)+gamma(i))); 
    clear diff
    end
    end
    y(:,:,T) = x(:,:,T); 
